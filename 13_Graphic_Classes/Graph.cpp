...
namespace Graph_lib 
{
	...
	{
		if (number_of_points() < 6) error("less than 6 points in a Hexagon");
		Closed_polyline::draw_lines();
	}

	Regular_hexagon::Regular_hexagon(Point p, int length) {
		// length in pixels
		add(Point(p.x, p.y-length));
		add(Point(p.x+length, p.y));
		add(Point(p.x, p.y + length));
		add(Point(p.x-length, p.y));
	}
